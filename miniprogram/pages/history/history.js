...
Page({
  ...
  async loadHistory() {
    ...
    try {
      ...
      this.setData({
        ...
        chartReady: items.length >= 2
      })
      if (items.length >= 2) wx.nextTick(() => this.drawChart())
    } catch (error) {
  ...
  drawChart() {
    ...
    query.select('#trendCanvas').fields({ node: true, size: true }).exec(result => {
      ...
      ctx.beginPath()
      ctx.moveTo(xy[0].x, height - padY)
      for (const point of xy) ctx.lineTo(point.x, point.y)
      ctx.lineTo(xy[xy.length - 1].x, height - padY)
      ctx.closePath()
      const fill = ctx.createLinearGradient(0, 0, 0, height)
      fill.addColorStop(0, 'rgba(28,31,36,0.12)')
      fill.addColorStop(1, 'rgba(28,31,36,0.01)')
      ctx.fillStyle = fill
      ctx.fill()

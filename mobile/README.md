...
这是一个面向 Android 的 Expo React Native + TypeScript 应用，采用浅灰背景、白色圆角卡片、分段校区切换和电量状态色。
...
- 低电量阈值可由用户在 5–40 度之间设置；允许通知后，应用会通过可用的通知能力提醒。
- App 会登记当前安装实例，用于接收低电量提醒和新版本提醒；收到更新通知后点击即可打开 HTTPS 下载地址。
- 设置“我的寝室”后会登记到服务端，由服务端按小时附近采样并保存最近 14 天历史；App 打开时同步服务器历史，充值跳变以橙色点和线段标识。
- 点击趋势预览可打开详情图；详情图横向可拖动，横轴是采样时间，纵轴是剩余电量（度）。
- 覆盖启动恢复、楼栋加载、查询中的加载态、网络/服务端错误、空楼栋、空结果和电量结果卡片。

## 后台能力边界

服务器端采样不依赖 App 是否在后台，因此 App 没有运行一整晚时，服务器仍会继续记录“我的寝室”电量。服务器任务按整点附近执行，不保证精确到每 60 分钟。

低电量远程推送需要 App 成功取得 Expo 推送令牌，并在服务端配置相应推送权限。未配置远程推送时，App 仍可在允许本地后台任务的情况下发送本地提醒；手机主动强行停止 App 时，本地提醒不能保证执行。
